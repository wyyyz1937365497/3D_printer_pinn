...
for t = 2:N_steps
    ...
    for mid = 1:config.n_machines
        % ========== 路径跟随控制 ==========
        if t <= path_steps && ~any(faulty_machines == mid && t > fault_start_step)
            % 正常机器跟随路径
        ...
        % 检查是否为温度传感器故障
        is_temp_fault = any(faulty_machines == mid & fault_types == 3 & t > fault_start_step);
        if is_temp_fault
            % 温度传感器漂移故障
        ...
        % 检查是否为机械松动故障
        is_mech_fault = any(faulty_machines == mid & fault_types == 2 & t > fault_start_step);
        if is_mech_fault
            % 机械松动：刚度降低50%
        ...
        % 检查是否为电机失步故障
        is_motor_fault = any(faulty_machines == mid & fault_types == 4 & t > fault_start_step);
        ...
        % ========== 挤出压力模型 ==========
        % 检查是否为喷嘴堵塞故障
        is_nozzle_fault = any(faulty_machines == mid & fault_types == 1 & t > fault_start_step);
        if is_nozzle_fault
            % 喷嘴部分堵塞：压力增加，挤出量减少
        ...
        % 故障惩罚
        fault_penalty = 0;
        if any(faulty_machines == mid & t > fault_start_step)
...
normal_machine = setdiff(1:config.n_machines, faulty_machines)(1);
